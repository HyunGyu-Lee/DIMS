...
import com.hst.dims.clients.customcontrols.ScheduleObject;
...
public class AdministratorMainController implements Initializable {
	...
	@FXML TextArea display_reciever;            // 선택된 메세지 수신자 목록 (STUDENT_NO, NAME)
	...
	ObservableList<String> alloption = FXCollections.observableArrayList("DEPARTMENT","컴퓨터정보공학과","철도경영물류학과 ","철도시설공학과 ","철도운전시스템공학과","철도전기전자공학과","철도차량시스템공학과");
	...
	public void createStudentSearch(JSONArray arr) // 영훈
	{
		...
		for(Object tt : arr)
		{
			...
			String name = t.get("DEPARTMENT").toString();
			...
			String snum = t.get("STUDENT_NO").toString();
...
//					calendar.initDate();
	...
	public void createWeabakManager(JSONArray arr)  // 영훈
	{
		...
		for(Object a : arr)
		{
			...
			String num = json.get("STUDENT_NO").toString();
	...
	public void StudentPlusMinusManagerList(JSONArray arr)
	{
		...
		for(Object a : arr)
		{
			...
			Label left1 = new Label(json.get("STUDENT_NO").toString());
			left1.setAlignment(Pos.CENTER);
			left1.setMaxHeight(30);
	...
	@FXML private void onSearch1()
	{
		...
		for(Object s : jarray)
		{
			...
			if(t.get("STUDENT_NO").toString().contains(searchKey) || t.get("이름").toString().contains(searchKey))
			{
				array.add(t);
			}
		}
	...
	@FXML private void onSearch2()
	{
		...
		for(Object a : jarray)
		{
			...
			if((aa.get("STUDENT_NO").toString().contains(searchKey)) ||(aa.get("이름").toString().contains(searchKey)) || aa.get("신청일자").toString().contains(searchKey)
	...
	@FXML private void onSearch3()
	{
		...
		{
			JSONObject aa = (JSONObject)a;
			if((aa.get("STUDENT_NO").toString().contains(searchKey)) ||(aa.get("이름").toString().contains(searchKey)))
			{
				jarr.add(aa);
			}
		}
	...
	@FXML public void onSearch4()
	{
		...
		{
			JSONObject aa = (JSONObject)a;
			if((aa.get("날짜").toString().contains(searchKey) || aa.get("STUDENT_NO").toString().contains(searchKey)) ||(aa.get("이름").toString().contains(searchKey)))
			{
				jarr.add(aa);
			}
		}
	...
	@FXML private void onAnswer()
	{
		...
		if(checkjarray.size() == 1)
		{
			...
			{
				JSONObject aa = (JSONObject)a;
				String num = aa.get("STUDENT_NO").toString();
	...
	private void createExistedSubmitUI(JSONObject dataBundle)
	{
		...
		for(Object o : (JSONArray)dataBundle.get("data-bundle"))
		{
			...
			Label id = new Label(target.get("STUDENT_NO").toString());
